...
class Circles {
    ...
    fun update(prevCentre:Vector2f, newCentre:Vector2f, newRadius:Float) : Circles {

        points.update(prevCentre, Vertex(
            posRadiusThickness  = Vector4f(newCentre.x, newCentre.y, newRadius, edgeThickness),
            edgeColour          = edgeColour.toVector4f(),
            fillColour          = fillColour.toVector4f()
        ))
    ...
    fun remove(centre:Vector2f) : Circles {
        points.remove(centre)
        pointsStale = true
        return this
    }
    fun clear() : Circles {
    ...
    private class Points : AbsTransferableArray() {
        ...
        fun update(centre:Vector2f, p:Vertex) {
            remove(centre)
            add(p)
        }
        fun remove(centre:Vector2f) {
            array.removeIf { it.posRadiusThickness.x == centre.x && it.posRadiusThickness.y == centre.y }
        }
        fun clear() {
            array.clear()
        }
