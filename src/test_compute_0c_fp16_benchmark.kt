...
fun main(args:Array<String>) {
    println("Testing Vulkan FP16 Compute...")

    val client = ComputeFP16Benchmark()
...
private class ComputeFP16Benchmark : VulkanClient(Parameters(headless = true)) {
    ...
    private var averageElapsedTime = 0L
    private var numSamplesTaken = 0
...
//        val features = vk.physicalDevice.getFeatures2KHR()
//        println("khr features = $features")
        ...
        pipeline
            ...
            .withShader("Benchmark/bench_fp16.comp")
            .build()
    ...
    private fun doCompute() {
        ...
        queryPool!!.getResults(0, 2, waitFor = false, includeAvailability = true).run {
            if(this.isNotEmpty()) {
                val elapsed = this[1]-this[0]
                averageElapsedTime += elapsed
                numSamplesTaken++
                println("Query timestamp results = $elapsed available=${this[2]} (average = ${averageElapsedTime/numSamplesTaken})")
            }
        }
    }
}